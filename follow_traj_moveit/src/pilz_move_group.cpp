...
int main(int argc, char* argv[])
{
  ...
  auto move_group_interface = MoveGroupInterface(node, "ur_on_rail_manipulator");
  ...
  auto const draw_trajectory_tool_path =
      [&moveit_visual_tools, jmg = move_group_interface.getRobotModel()->getJointModelGroup("ur_on_rail_manipulator")](
...

  {
    ...
    move_group_interface.setNamedTarget("elbow_up");
    plan_and_execute("[PTP] Ready");
  }

  {
    ...
    auto const pre_grasp_pose = [] {
      ...
      msg.pose.position.x = -0.260;
      msg.pose.position.y = -0.058;
...

  {
    ...
    auto const goal_pose = [] {
      ...
      msg.pose.orientation.z = 0.0;
      msg.pose.orientation.w = 0.0;
      msg.pose.position.x = 0.201;
      msg.pose.position.y = 0.137;
      msg.pose.position.z = 0.849;
      return msg;
    }();
    move_group_interface.setPoseTarget(goal_pose, "tool0");
    ...
    auto const center_pose = [] {
      ...
      msg.pose.orientation.z = 0.0;
      msg.pose.orientation.w = 0.0;
      msg.pose.position.x = 0.073;
      msg.pose.position.y = 0.271;
      msg.pose.position.z = 0.849;
      return msg;
    }();
    moveit_msgs::msg::Constraints constraints;
    constraints.name = "interim";  // Change to "interim" to use an intermediate point on arc instead.
